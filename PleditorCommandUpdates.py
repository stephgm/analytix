...
class Pleditor(QMainWindow):
    ...
    def preview(self):
        ...
        try:
            if self.fname:
                ...
                self.dynamic_canvas = pltr.createPlot('',RETURN=True,CANVAS=self.dynamic_canvas)
    #                self.dynamic_canvas = FigureCanvas(myfig)
    ...
    def resizeTrees(self):
        for tree in self.TreeList:
            tree.setSizeAdjustPolicy(tree.AdjustToContents)
...
#            if not parent:
            if len(dkeymap) == 1 and 'fig' in dkeymap:
                ...
                self.TreeList.append(QTreeWidget())
                self.currentTreeWidget = self.TreeList[-1]
                ...
                tablelayout = QGridLayout()
                self.TreeList.append(QTreeWidget())
                self.OGLinesTW = self.TreeList[-1]
...
#                        le.setValidator(doublevalidator)
                    ...
                    if d[k]:
                        ...
                        for i,cmd in enumerate(d[k]):
                            ...
                            commandname.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=totalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index))
                            for arg in cmd['args']:
...
#                                if not isinstance(arg,np.ndarray):
                                ...
                                argwidget.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=totalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index))
                            ...
                            for kwarg in cmd['kwargs']:
                                ...
                                if kwarg in ['color','closed','fill','ls','transform']:
                                    ...
                                    kwargVal.currentIndexChanged.connect(lambda trash, plkeymap=dkeymap,plwidget=totalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index=i))
                                else:
                                    kwargVal = QLineEdit(str(cmd['kwargs'][kwarg]))
                                    kwargVal.setValidator(kwargvalidator)
                                    kwargVal.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=totalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index))
                            ...
                            rargpushbutton.clicked.connect(lambda trash,listwidget=commandargs,plwidget=totalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.removeArg(listwidget,plwidget,plkeymap,pldictkey,index))
                            rkwargpushbutton.clicked.connect(lambda trash,listwidget=commandkwargs,plwidget=totalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.removeKwarg(listwidget,plwidget,plkeymap,pldictkey,index))
                            argpushbutton.clicked.connect(lambda trash,listwidget=commandargs,plwidget=totalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.addArg(listwidget,plwidget,plkeymap,pldictkey,index))
                            kwargpushbutton.clicked.connect(lambda trash, listwidget=commandkwargs,plwidget=totalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.addKwarg(listwidget,plwidget,plkeymap,pldictkey,index))
                    ...
                    if d[k]:
                        ...
                        for i,pcmd in enumerate(d[k]):
                            ...
                            pcommandname.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=ptotalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index))
...
#                                argwidget.setReadOnly(True)
                                pargwidget.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=ptotalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index))
                            ...
                            for ii,pkwarg in enumerate(pcmd['kwargs']):
                                ...
                                if pkwarg in ['color','closed','fill','ls','transform']:
                                    ...
                                    pkwargVal.currentIndexChanged.connect(lambda trash, plkeymap=dkeymap,plwidget=ptotalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index))
                                else:
                                    pkwargVal = QLineEdit(str(pcmd['kwargs'][pkwarg]))
                                    pkwargVal.setValidator(pkwargvalidator)
                                    pkwargVal.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=ptotalwidget,pldictkey=k,index=i:self.rebuildCommand(plkeymap,plwidget,pldictkey,index))
                            ...
                            prargpushbutton.clicked.connect(lambda trash,listwidget=pcommandargs,plwidget=totalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.removeArg(listwidget,plwidget,plkeymap,pldictkey,index))
                            prkwargpushbutton.clicked.connect(lambda trash,listwidget=pcommandkwargs,plwidget=totalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.removeKwarg(listwidget,plwidget,plkeymap,pldictkey,index))
                            pargpushbutton.clicked.connect(lambda trash,listwidget=pcommandargs,plwidget=ptotalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.addArg(listwidget,plwidget,plkeymap,pldictkey,index))
                            pkwargpushbutton.clicked.connect(lambda trash, listwidget=pcommandkwargs,plwidget=ptotalwidget,plkeymap=dkeymap,pldictkey=k,index=i:self.addKwarg(listwidget,plwidget,plkeymap,pldictkey,index))
...
#                parent.setFlags(parent.flags())
    ...
    def removeArg(self,widget,totalwidget,keymap,key,index):
        for SelectedItem in widget.selectedItems():
            widget.takeItem(widget.row(SelectedItem))
        self.rebuildCommand(keymap,totalwidget,key,index)
        self.resizeTrees()
    
    def removeKwarg(self,widget,totalwidget,keymap,key,index):
        for SelectedItem in widget.selectedItems():
            widget.takeItem(widget.row(SelectedItem))
        self.rebuildCommand(keymap,totalwidget,key,index)
        self.resizeTrees()
    
    def addArg(self,widget,totalwidget,dkeymap,key,index):
        itemN = QListWidgetItem()
        le = QLineEdit()
        itemN.setSizeHint(le.sizeHint())
        widget.addItem(itemN)
        widget.setItemWidget(itemN,le)
        le.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=totalwidget,plkey=key,plindex=index:self.rebuildCommand(plkeymap,plwidget,plkey,plindex))
        self.resizeTrees()
    ...
    def addKwarg(self,widget,totalwidget,dkeymap,key,index,labelName=None,value=None):
        ...
        if labelName in ['color','closed','fill','ls','transform']:
            ...
            kwargVal.currentIndexChanged.connect(lambda trash, plkeymap=dkeymap,plwidget=totalwidget,pldictkey=key,plindex=index:self.rebuildCommand(plkeymap,plwidget,pldictkey,plindex))
        else:
            if not value:
                kwargVal = QLineEdit()
            else:
                kwargVal = QLineEdit(value)
            kwargVal.editingFinished.connect(lambda plkeymap=dkeymap,plwidget=totalwidget,plkey=key,plindex=index:self.rebuildCommand(plkeymap,plwidget,plkey,plindex))
    ...
    def addCommand(self,keymap,key,layout,totalwidget):
        ...
        commandname.editingFinished.connect(lambda plkeymap=keymap,plwidget=totalwidget,plkey=key,index=len(self.getFromDict(keymap)[key]):self.rebuildCommand(plkeymap,plwidget,plkey,index))
        commandargs = QListWidget()
        commandkwargs = QListWidget()
        toplevellayout.addWidget(commandname,1,0)
        toplevellayout.addWidget(commandargs,1,1)
        ...
        kwargpushbutton.clicked.connect(lambda trash, listwidget=commandkwargs,plwidget=totalwidget,plkeymap=keymap,pldictkey=key,index=len(self.getFromDict(keymap)[key]):self.addKwarg(listwidget,plwidget,plkeymap,pldictkey,index))
        nwidget.setLayout(toplevellayout)
        layout.addWidget(nwidget)
        self.getFromDict(keymap)[key].append({'cmd':'','args':[],'kwargs':{}})
        self.resizeTrees()
    ...
    def addPatch(self,keymap,key,layout,totalwidget):
        ...
        if patchtype in ['Rectangle','Ellipse']:
            ...
            self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'height','1.0')
            self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'angle','0.0')
            patchdict = dict(xy=(0.,0.),width=1.0,height=1.0,angle=0.)
        elif patchtype == 'Circle':
            self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'xy','(0.0,0.0)')
            self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'radius','1.0')
            patchdict = dict(xy=(0.,0.),radius=1.0)
        elif patchtype == 'Polygon':
            self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'xy','(0.0,0.0)')
            self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'closed','True')
            patchdict = dict(xy=(0.,0.),closed=True)
        self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'fill','True')
        self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'alpha','1.0')
        self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'color','black')
        self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'ls','--')
        self.addKwarg(commandkwargs,totalwidget,keymap,key,index,'zorder','1')
        alldict = dict(fill=True,alpha=1.0,color="'''black'''",ls="'''--'''",zorder=1)
...
#        self.rebuildCommands(keymap,totalwidget,key)
    ...
    def changePatch(self,key,keymap,patchcombo,arglist,kwarglist,totalwidget,index):
        ...
        if patchtype in ['Rectangle','Ellipse']:
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'xy','(0.0,0.0)')
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'width','1.0')
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'height','1.0')
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'angle','0.0')
            patchdict = dict(xy=(0.,0.),width=1.0,height=1.0,angle=0.) 
        elif patchtype == 'Circle':
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'xy','(0.0,0.0)')
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'radius','1.0')
            patchdict = dict(xy=(0.,0.),radius=1.0)
        elif patchtype == 'Polygon':
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'xy','(0.0,0.0)')
            self.addKwarg(kwarglist,totalwidget,keymap,key,index,'closed','True')
            patchdict = dict(xy=(0.,0.),closed=True)
        self.addKwarg(kwarglist,totalwidget,keymap,key,index,'fill','True')
        self.addKwarg(kwarglist,totalwidget,keymap,key,index,'alpha','1.0')
        self.addKwarg(kwarglist,totalwidget,keymap,key,index,'color','black')
        self.addKwarg(kwarglist,totalwidget,keymap,key,index,'ls','-')
        self.addKwarg(kwarglist,totalwidget,keymap,key,index,'zorder','1')
        alldict = dict(fill=True,alpha=1.0,color="'''black'''",ls="'''--'''",zorder=1)
        patchdict.update(alldict)
#        self.rebuildCommands(keymap,totalwidget,key)
